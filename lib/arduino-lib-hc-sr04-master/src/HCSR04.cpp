...
double UltraSonicDistanceSensor::measureDistanceCm(unsigned long echoTime) {
    //Using the approximate formula 19.307°C results in roughly 343m/s which is the commonly used value for air.
    return measureDistanceCm(19.307, echoTime);
}

double UltraSonicDistanceSensor::measureDistanceCm(float temperature, unsigned long echoTime) {
    ...
    double distanceCm = echoTime / 2.0 * speedOfSoundInCmPerMs;
